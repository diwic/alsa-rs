    ...
    fn get(&self) -> Result<Vec<pollfd>> {
        ...
        if self.fill(&mut v)? != v.len() {
            Err(Error::unsupported(
                "did not fill the poll descriptors array",
            ))
        } else {
            Ok(v)
        }
...
impl Descriptors for pollfd {
    ...
    fn revents(&self, a: &[pollfd]) -> Result<Flags> {
        Ok(Flags::from_bits_truncate(a[0].revents))
    }
}
...
pub fn poll(fds: &mut [pollfd], timeout: i32) -> Result<usize> {
    ...
    if r >= 0 {
        ...
        from_code("poll", -io::Error::last_os_error().raw_os_error().unwrap())
            .map(|_| unreachable!())
    }
}
...
pub fn poll_all<'a>(
    desc: &[&'a dyn Descriptors],
    timeout: i32,
) -> Result<Vec<(&'a dyn Descriptors, Flags)>> {

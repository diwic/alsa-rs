...
use crate::{alsa, Card};
use libc::{c_int, c_short, c_uint, pollfd};
...
impl HCtl {
    ...
    pub fn open(c: &CStr, nonblock: bool) -> Result<HCtl> {
        ...
        acheck!(snd_hctl_open(&mut r, c.as_ptr(), flags)).map(|_| HCtl(r))
    }
...
impl poll::Descriptors for HCtl {
    ...
    fn fill(&self, p: &mut [pollfd]) -> Result<usize> {
        let z =
            unsafe { alsa::snd_hctl_poll_descriptors(self.0, p.as_mut_ptr(), p.len() as c_uint) };
        from_code("snd_hctl_poll_descriptors", z).map(|_| z as usize)
    }
    fn revents(&self, p: &[pollfd]) -> Result<poll::Flags> {
        let mut r = 0;
        let z = unsafe {
            alsa::snd_hctl_poll_descriptors_revents(
                self.0,
        ...
        from_code("snd_hctl_poll_descriptors_revents", z)
...
impl<'a> Iterator for ElemIter<'a> {
    ...
    fn next(&mut self) -> Option<Elem<'a>> {
        self.1 = if self.1.is_null() {
            unsafe { alsa::snd_hctl_first_elem((self.0).0) }
        } else {
            unsafe { alsa::snd_hctl_elem_next(self.1) }
        };
        if self.1.is_null() {
            None
        } else {
            Some(Elem(self.0, self.1))
        }
    }
}
...
fn print_hctls() {
    for a in super::card::Iter::new().map(|x| x.unwrap()) {
        use std::ffi::CString;
        let h = HCtl::open(
            &CString::new(format!("hw:{}", a.get_index())).unwrap(),
            false,
        )
        .unwrap();
        h.load().unwrap();
...
fn print_jacks() {
    for a in super::card::Iter::new().map(|x| x.unwrap()) {
        ...
        for b in h.elem_iter() {
            let id = b.get_id().unwrap();
            if id.get_interface() != super::ctl_int::ElemIface::Card {
                continue;
            }
            let name = id.get_name().unwrap();
            ...
            if name.ends_with(" Phantom Jack") {
                ...
                println!(
                    "{} is {}",
                    &name[..name.len() - 5],
                    if b.read().unwrap().get_boolean(0).unwrap() {
                        "plugged in"
                    } else {
                        "unplugged"
                    }

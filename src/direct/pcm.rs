...
use crate::{pcm, Direction, PollDescriptors};
use libc;
...
impl SyncPtrStatus {
    ...
    pub unsafe fn sync_ptr(
        fd: RawFd,
        hwsync: bool,
        appl_ptr: Option<pcm::Frames>,
        avail_min: Option<pcm::Frames>,
    ) -> Result<Self> {
        ...
        if (i >= (pcm::State::Open as snd_pcm_state_t))
            && (i <= (pcm::State::Disconnected as snd_pcm_state_t))
        {
            Ok(SyncPtrStatus(data.s.status))
        } else {
            Err(Error::unsupported(
                "SNDRV_PCM_IOCTL_SYNC_PTR returned broken state",
            ))
        }
    }

    pub fn hw_ptr(&self) -> pcm::Frames {
        self.0.hw_ptr as pcm::Frames
    }
    pub fn state(&self) -> pcm::State {
        unsafe { mem::transmute(self.0.state as u8) } /* valid range checked in sync_ptr */
    }
...
fn pcm_to_fd(p: &pcm::PCM) -> Result<RawFd> {
    ...
    if c != 1 {
        return Err(Error::unsupported(
            "snd_pcm_poll_descriptors returned wrong number of fds",
        ));
    }
    Ok(fds[0].fd)
}
...
impl Status {
    ...
    pub fn state(&self) -> pcm::State {
        unsafe {
            ...
            assert!(
                (i >= (pcm::State::Open as snd_pcm_state_t))
                    && (i <= (pcm::State::Disconnected as snd_pcm_state_t))
            );
    ...
    pub fn hw_ptr(&self) -> pcm::Frames {
        unsafe { ptr::read_volatile(&(*self.0.ptr).hw_ptr) as pcm::Frames }
    }
    ...
    pub fn htstamp(&self) -> libc::timespec {
        unsafe { ptr::read_volatile(&(*self.0.ptr).tstamp) }
    }
    ...
    pub fn audio_htstamp(&self) -> libc::timespec {
        unsafe { ptr::read_volatile(&(*self.0.ptr).audio_tstamp) }
    }
}
...
impl Control {
    ...
    pub fn appl_ptr(&self) -> pcm::Frames {
        unsafe { ptr::read_volatile(&(*self.0.ptr).appl_ptr) as pcm::Frames }
    }
    ...
    pub fn avail_min(&self) -> pcm::Frames {
        unsafe { ptr::read_volatile(&(*self.0.ptr).avail_min) as pcm::Frames }
    }
...
impl<S> SampleData<S> {
    pub fn new(p: &pcm::PCM) -> Result<Self> {
        ...
        Ok(SampleData {
            mem: DriverMemory::new(
                fd,
                (bufsize as usize) * (channels as usize),
...
impl MmapDir for Playback {
    ...
    fn avail(hwptr: Frames, applptr: Frames, buffersize: Frames, boundary: Frames) -> Frames {
        ...
        if r as usize >= boundary as usize {
            r.wrapping_sub(boundary)
        } else {
            r
        }
    }
...
impl<S> RawSamples<S> {
    ...
    pub fn samples(&self) -> isize {
        self.frames as isize * (self.channels as isize)
    }
...
pub(crate) fn new_mmap<S, D: MmapDir>(p: &pcm::PCM) -> Result<MmapIO<S, D>> {
    MmapIO::new(p)
}
    ...
    pub fn data_ptr(&self) -> (RawSamples<S>, Option<RawSamples<S>>) {
        ...
        let p = unsafe {
            self.data
                .mem
                .ptr
                .offset(offs as isize * self.data.channels as isize)
        };
        (
            RawSamples {
                ptr: p,
...
impl<'a, S: 'static + Copy> CaptureIter<'a, S> {
    fn handle_max(&mut self) {
        ...
        if let Some(p2) = self.next_p.take() {
            ...
            self.m
                .commit((self.read_samples / self.samples.channels as isize) as Frames);
            self.read_samples = 0;
...
impl<'a, S: 'static> Drop for CaptureIter<'a, S> {
    fn drop(&mut self) {
        self.m
            .commit((self.read_samples / self.m.data.channels as isize) as Frames);
    }
}
...
fn record_from_plughw_mmap() {
    ...
    let ss =
        unsafe { SyncPtrStatus::sync_ptr(pcm_to_fd(&pcm).unwrap(), false, None, None).unwrap() };
    assert_eq!(ss.state(), State::Prepared);
...
fn playback_to_plughw_mmap() {
    ...
    println!("{:?}", m);
    let mut i = (0..(m.buffer_size() * 2))
        .map(|i| (((i / 2) as f32 * 2.0 * ::std::f32::consts::PI / 128.0).sin() * 8192.0) as i16);
    m.write(&mut i);

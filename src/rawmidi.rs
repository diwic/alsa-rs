...
impl Drop for Info {
    fn drop(&mut self) {
        unsafe { alsa::snd_rawmidi_info_free(self.0) };
    }
}
...
impl Info {
    ...
    fn subdev_count(c: &Ctl, device: c_int) -> Result<(i32, i32)> {
        ...
        Ok((
            unsafe { alsa::snd_rawmidi_info_get_subdevices_count(o.0) as i32 },
            unsafe { alsa::snd_rawmidi_info_get_subdevices_count(i.0) as i32 },
        ))
    }

    pub fn get_device(&self) -> i32 {
        unsafe { alsa::snd_rawmidi_info_get_device(self.0) as i32 }
    }
    pub fn get_subdevice(&self) -> i32 {
        unsafe { alsa::snd_rawmidi_info_get_subdevice(self.0) as i32 }
    }
    pub fn get_stream(&self) -> super::Direction {
        if unsafe { alsa::snd_rawmidi_info_get_stream(self.0) } == alsa::SND_RAWMIDI_STREAM_OUTPUT {
            super::Direction::Playback
        } else {
            super::Direction::Capture
        }
    }
...
impl Status {
    pub fn get_avail(&self) -> usize {
        unsafe { alsa::snd_rawmidi_status_get_avail(self.0 as *const _) }
    }
    pub fn get_xruns(&self) -> usize {
        unsafe { alsa::snd_rawmidi_status_get_xruns(self.0 as *const _) }
    }
}

impl Drop for Status {
    fn drop(&mut self) {
        unsafe { alsa::snd_rawmidi_status_free(self.0) };
    }
}

impl<'a> Iter<'a> {
    pub fn new(c: &'a Ctl) -> Iter<'a> {
        Iter {
            ctl: c,
            device: -1,
            in_count: 0,
    ...
    fn next(&mut self) -> Option<Result<Info>> {
        ...
        if self.current - self.in_count < self.out_count {
            ...
            return Some(Info::from_iter(
                self.ctl,
                self.device,
                self.current - 1 - self.in_count,
                Direction::Playback,
            ));
        }

        let r = acheck!(snd_ctl_rawmidi_next_device(
            ctl_ptr(self.ctl),
            &mut self.device
        ));
        match r {
...
impl Rawmidi {
    ...
    pub fn drop(&self) -> Result<()> {
        acheck!(snd_rawmidi_drop(self.0)).map(|_| ())
    }
    pub fn drain(&self) -> Result<()> {
        acheck!(snd_rawmidi_drain(self.0)).map(|_| ())
    }
...
impl poll::Descriptors for Rawmidi {
    ...
    fn fill(&self, p: &mut [pollfd]) -> Result<usize> {
        let z = unsafe {
            alsa::snd_rawmidi_poll_descriptors(self.0, p.as_mut_ptr(), p.len() as c_uint)
        };
    ...
    fn revents(&self, p: &[pollfd]) -> Result<poll::Flags> {
        let mut r = 0;
        let z = unsafe {
            alsa::snd_rawmidi_poll_descriptors_revents(
                self.0,
        ...
        from_code("snd_rawmidi_poll_descriptors_revents", z)

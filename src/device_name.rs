...
impl Drop for HintIter {
    fn drop(&mut self) {
        unsafe {
            alsa::snd_device_name_free_hint(self.0);
        }
    }
}
...
impl HintIter {
    ...
    pub fn new(card: Option<&Card>, iface: &CStr) -> Result<HintIter> {
        ...
        acheck!(snd_device_name_hint(cnr, iface.as_ptr(), &mut p)).map(|_| HintIter(p, 0))
    }
...
impl Hint {
    ...
    fn new(p: *const c_void) -> Hint {
        ...
        Hint {
            name: Hint::get_str(p, "NAME"),
            desc: Hint::get_str(p, "DESC"),
            direction: d,
        }
    }
}

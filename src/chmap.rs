...
impl Chmap {
    fn set_channels(&mut self, c: libc::c_uint) {
        unsafe { (*self.0).channels = c }
    }
    fn as_slice_mut(&mut self) -> &mut [libc::c_uint] {
        unsafe {
            slice::from_raw_parts_mut((*self.0).pos.as_mut_ptr(), (*self.0).channels as usize)
        }
    }
...
impl<'a> From<&'a [ChmapPosition]> for Chmap {
    fn from(a: &'a [ChmapPosition]) -> Chmap {
        let p = unsafe {
            libc::malloc(
                (mem::size_of::<alsa::snd_pcm_chmap_t>() + mem::size_of::<libc::c_uint>() * a.len())
                    as libc::size_t,
            )
        };
        if p.is_null() {
        ...
        for (i, v) in r.as_slice_mut().iter_mut().enumerate() {
            *v = a[i] as libc::c_uint
        }
        r
    }
}
...
impl<'a> From<&'a Chmap> for Vec<ChmapPosition> {
    fn from(a: &'a Chmap) -> Vec<ChmapPosition> {
        a.as_slice()
            .iter()
            .map(|&v| ChmapPosition::from_c_int(v as libc::c_int, "").unwrap())
            .collect()
    }
}

pub fn chmap_new(a: *mut alsa::snd_pcm_chmap_t) -> Chmap {
    Chmap(a, true)
}
pub fn chmap_handle(a: &Chmap) -> *mut alsa::snd_pcm_chmap_t {
    a.0
}
...
impl Drop for ChmapsQuery {
    fn drop(&mut self) {
        unsafe { alsa::snd_pcm_free_chmaps(self.0) }
    }
}

pub fn chmaps_query_new(a: *mut *mut alsa::snd_pcm_chmap_query_t) -> ChmapsQuery {
    ChmapsQuery(a, 0)
}
...
impl Iterator for ChmapsQuery {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        self.1 += 1;
        let t = ChmapType::from_c_int(unsafe { (*p).type_ } as libc::c_int, "snd_pcm_query_chmaps")
            .unwrap();
